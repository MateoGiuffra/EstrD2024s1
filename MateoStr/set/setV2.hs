...
-- O(n) # ya que se usa sumarSiNoExiste de costo lineal sobre xs 
addS :: Eq a => a -> Set a -> Set a
-- Dados un elemento y un conjunto, agrega el elemento al conjunto. 
addS e (S (xs) n) = (S (e:xs) (sumarSiNoExiste n e xs))

-- O(n) # porque se usa elem en xs, siendo xs la cantidad de elementos que representa n en (S xs n) 
sumarSiNoExiste :: Eq => a -> Int ->a  ->[a] -> Int->
sumarSiNoExiste n e xs = if elem e xs then n else n+1


--O(n) # de costo n por elem, siendo n la longitud de xs en (S xs _)
belongs :: Eq a => a -> Set a -> Bool
-- Dados un elemento y un conjunto indica si el elemento pertenece al conjunto.
belongs e (S xs _) = elem e xs 
...
-- O(n^2) # ya que se usa sinRepetidos 
setToList :: Eq a => Set a -> [a]
-- Dado un conjunto devuelve una lista con todos los elementos distintos del conjunto.
setToList (S xs _) = sinRepetidos xs 

-- O( n # siendo n la longitud de la lista ya que se hace RE sobre la misma.   
-- * por cada elemento de la lista
-- ( n # se usa elem sobre la longitud de la lista
-- ))
-- O(n^2)
