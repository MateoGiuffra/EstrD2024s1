...
-- REVERSION DE EFICIENCIA POR NUEVOS COSTOS: 
...
-- EFICIENCIA: O(K+(K*Log K))
-- O(K # siendo K la cantidad de elementos del map ya que se hace keys sobre el mismo. 
--  + 
--  K*(log K) # por lookupsM sobre el map y sus llaves (mismo K).
-- O(K+(K*Log K))
-- O(K*Log K) queda el mas grande???
...
-- log K # por lookupM sobre la recursion. 
--))
lookupsM []     map = []  
lookupsM (k:ks) map = lookupM k map : (lookupsM ks map )

todasAsociadas :: Eq k => [k] -> Map k v -> Bool
-- Proposito: Indica si en el map se encuentran todas las claves dadas.
-- EFICIENCIA: O(n*Log K)
...
-- log n  # por assocM sobre la RE, siendo n la cantidad de elementos del map que es igual a la cantidad de (k,v) de kvs
-- ))
listToMap []          = emptyM 
...
-- *       # por cada k de ks se hace:
-- (log K  # porque se hace lookupM de k sobre map, siendo K la cantidad de elementos del map.  
-- + 
...
-- HASTA ACA porque no se si es log n cuando uso una funcion de ese costo sobre la RE o es log K porque puede ser que sean 
-- distintos numeros (cantidad de elementos / longitud de la lista)
